...
def build_fev1_fef2575_o2sat_noise_with_factor_graph(app):
    ...
    def content(
        ...
        # O2SatFFA
        fig_o2satffa = make_subplots(rows=1, cols=1)
        ih.plot_histogram(
            fig_o2satffa,
            O2SatFFA,
            res_o2satffa.values,
        ...
        fig_o2satffa.update_layout(
            showlegend=False,
            height=height_posterior,
            width=width,
            font=dict(size=font_size),
            bargap=0.01,
            margin=dict(l=0, r=0, b=0, t=0),
        ...
        # uO2Sat
        fig_uo2sat = make_subplots(rows=1, cols=1)
        ih.plot_histogram(
            fig_uo2sat,
            uO2Sat,
            res_uo2sat.values,
...
def build_fev1_fef2575_o2sat_noise_with_factor_graph_log(app):
    ...
    def content(
        ...
        (
            ...
            O2Sat,
            ecFEF2575prctecFEV1,
        ) = mb.o2sat_fev1_fef2575_point_in_time_model_noise_shared_healthy_vars_log(
            height,
            age,
            sex,
            ia_prior,
...
def build_fev1_fef2575_o2sat_noise_with_factor_graph_debug(app):
    ...
    def content(
        ...
        posterior_plot_location_dict = {
            HFEV1.name: (HFEV1, 2, 1, fev_min, fev_max),
            uFEV1.name: (uFEV1, 5, 1, fev_min, fev_max),
            HO2Sat.name: (HO2Sat, 2, 5, o2sat_min, o2sat_max),
            AR.name: (AR, 5, 3, AR.a, AR.b),
            O2SatFFA.name: (O2SatFFA, 8, 5, o2sat_min, o2sat_max),
            IA.name: (IA, 11, 3, ia_min, ia_max),
        ...
        viz_layout = [
            ...
            [None, None, m_plot, None, None],  # 7
            [None, None, m_plot, None, None],  # 8
            [m_plot, None, m_plot, None, m_plot],  # 9
            [None, None, None, m_plot, None],  # 10
            [None, None, None, None, None],  # 11
            [None, None, None, None, m_plot],  # 12
            [None, None, None, None, None],  # 13
        ...
        ih.plot_histogram(fig, HFEV1, HFEV1.cpt, fev_min, fev_max, 1, 1, None, "green")
        # factor - HFEV1
        key = get_key(f"['{uFEV1.name}', '{HFEV1.name}', '{AR.name}']", HFEV1.name)
        ih.plot_histogram(
            fig, HFEV1, messages[key], fev_min, fev_max, 4, 1, None, "gray"
        )
        # ecFEV1 factor to uFEV1
        key = get_key(f"['{ecFEV1.name}', '{uFEV1.name}']", uFEV1.name)
        ih.plot_histogram(
            fig, uFEV1, messages[key], fev_min, fev_max, 7, 1, None, "gray"
        )
        # FEV1 obs
        ih.plot_histogram(
            fig,
