...
def get_patient_ex_labels(patient_antibioticsdata, patientsdata, numdays_before_ab_start_is_exacerbated=7,
    ...
    for i in range(0, len(patient_antibioticsdata)):
        ...
        if i == 0:
            ...
            is_exacerbated_start = _get_is_exacerbated_start(antibiotic_start, study_start - one_day,
                                                             numdays_before_ab_start_is_exacerbated)
        else:
            is_exacerbated_start = _get_is_exacerbated_start(antibiotic_start, prev_antibiotic_end,
                                                             numdays_before_ab_start_is_exacerbated)
        exacerbated_period_agg.extend(_create_date_range(is_exacerbated_start, _get_is_exacerbated_end(antibiotic_end)))
        ...
        if i == 0:
            ...
            not_exacerbated_end = _get_not_exacerbated_end(antibiotic_start, study_start - one_day,
                                                           numdays_before_ab_start_not_exacerbated)
        else:
            not_exacerbated_start = _get_not_exacerbated_start(prev_antibiotic_end)
            not_exacerbated_end = _get_not_exacerbated_end(antibiotic_start, prev_antibiotic_end,
                                                           numdays_before_ab_start_not_exacerbated)

        not_exacerbated_period_agg.extend(_create_date_range(not_exacerbated_start, not_exacerbated_end))
...
def _get_is_exacerbated_start(antibiotic_start, prev_antibiotics_end, numdays_before_start=7):
    # The patient cannot be labelled as exacerbated before the end of the previous antibiotic (or on same day)
    return max(prev_antibiotics_end + one_day, antibiotic_start - timedelta(days=numdays_before_start))
...
def _get_not_exacerbated_end(antibiotic_start, prev_antibiotics_end, numdays_before_start=21):
